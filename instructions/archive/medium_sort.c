...
void	medium_sort(t_list **stack_a, t_list **stack_b, t_op_node **ops_head)
{
	...
	chunk_count = (stack_len + sqrt_approx(stack_len) - 1)
		/ sqrt_approx(stack_len);
	current_chunk = 0;
	while (current_chunk < chunk_count && stack_length(*stack_a) > 0)
	{
		range_start = current_chunk * sqrt_approx(stack_len);
		range_end = range_start + sqrt_approx(stack_len) - 1;
		if (range_end >= stack_len)
			range_end = stack_len - 1;
		if ((*stack_a)->content >= range_start
			&& (*stack_a)->content <= range_end)
		{
			pb(stack_a, stack_b);
