...
void	benchmark_output(int *array, int length, t_flags flags, t_op_node *ops)
{
	...
	write(2, "%\nStrategy: ", 12);
	write(2, strategy_name(flags.strategy),
		ft_strlen(strategy_name(flags.strategy)));
	write(2, " (", 2);
	write(2, complexity_class(flags.strategy),
		ft_strlen(complexity_class(flags.strategy)));
	write(2, ")\nTotal operations: ", 20);
